...
//FUNÇÃO NOVA, QUE SERÁ ACRESCIDA NA BIBLIOTECA
...
function transfer360(px, py, radius, rotation, escale5, escale2, escale1){
	var px; py; radius; rotation; escale1; escale2; escale5;
	var corLinha = '#ced6e0';
	var corLinha2 = '#dcdde1';
	line(px - radius, py, px + radius, py, 1, corLinha2);
	arcCircle2(px, py, radius, 0, 360, 1.5, corLinha);
	arcCircle2(px, py, radius - 10, 0, 360, 1, corLinha2);
	lineAngle(px, py + radius, 2*radius, -90, 1, corLinha2);
	//	
	for(var i = 1; i <= 35; i++){
		lineReverse(px, py, radius, 10, -i*10, 1.5, corLinha);
	}
	if(escale5 == true){
		for(var i = 5; i <= 355; i=i+10){
			lineReverse(px, py, radius, 5, -i, 1, corLinha);
		}
	}
	
	if(escale2 == true){
		for(var i = 2; i <= 180; i=i+2){
			lineReverse(px, py, radius, 10, -i, 1, corLinha);
		}
	}	
...
function transfer180(px, py, radius, rotation, escale5, escale2, escale1){
	var px; py; radius; rotation; escale1; escale2; escale5;
	var corLinha = '#ced6e0';
	var corLinha2 = '#dcdde1';
	line(px - radius, py, px + radius, py, 1, corLinha2);
...
function keyOnOf(px, py, status){
	...
	rectangle(px, py - 5, 40, 10, 0, 0, nocolor, 'rgb(255,255,255)');
...
function battery(px, py, rotation){
	...
	ctx.translate(-px,-py);
	//
	rectangle(px + 4 , py - 4, 12, 5, 1, 0,'#341f97', '#c8d6e5');
	rectangle(px, py, 20, 60, 1, 0, '#341f97', '#c8d6e5');
...
function person(px, py, rotation, scale, colorfill){
	...
	ctx.scale(scale,scale);
	//
	rectangle(px, py - 20, 5, 20, 0, 0, nocolor, colorfill);
	rectangle(px + 7, py - 20, 5, 20, 0, 0, nocolor, colorfill);
	rectangle(px, py - 22 - 19, 12, 20, 0, 0, nocolor, colorfill);
	rectangle(px - 6, py - 22 - 19, 5, 20, 0, 0, nocolor, colorfill);
	rectangle(px + 13, py - 22 - 19, 5, 20, 0, 0, nocolor, colorfill);
	circle(px + 6, py - 29 - 19, 5, 0, nocolor, colorfill);
	//
	ctx.restore();
}
...
function error(x, y, color){
	rectangle(x, y, 30, -5, 1.5, 45, color, color);
	rectangle(x, y + 18, 30, 5, 1.5, -45, color, color);		
}
